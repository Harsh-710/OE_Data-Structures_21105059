...
//Creating structure of the Node of tree
class Node{
    public:
        int data;
        Node*left;
...
But if we look at the total space required by the BST, then it is
much more than that of the array as each node of the tree requires
more memory than a single cell in the array.
For eg. each integer is allocated 4 bytes of memory, so an array of
integers of size 'n' will require 4*n bytes of memory. However, a
single node of BST contains an integer data, and two node pointers
which add upto 4 + 8 + 8 = 20 bytes of memory for a single node.
So, the total memory required by the BST will be 20*n bytes.
*/
